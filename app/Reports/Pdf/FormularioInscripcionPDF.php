...
use App\Helpers\FpdfGlobal;
use App\Interfaces\LetterDownloadInterface;
use Codedge\Fpdf\Fpdf\Fpdf;

class FormularioInscripcionPDF extends FpdfGlobal implements LetterDownloadInterface
{
    public function download($data)
    {
        ...
        $url = $data['photo'];  // La URL original con los parámetros
        $tamañoPapel = 220;
        $nacionalidad = $data['isBolivian'] ? "Boliviano" : "Extranjero";
        ...
        $this->fpdf->Image($url, $this->width + 10, 10, 50, 20);
        ...
        $this->fpdf->Cell(10, 10, $this->utf8_decode("Diplomado en Tecnología, Diseño de
Rehabilitación de Carreteras y Gestión Ambiental."), 0, 0, 'L');
        $this->fpdf->Ln(5);

        $this->fpdf->SetX(10);
        $this->fpdf->Cell($tab, 10, $this->utf8_decode("VERSION:"), 0, 0, 'L');
        $this->fpdf->Cell(10, 10, $this->utf8_decode("Presencial"), 0, 0, 'L');
        ...
        $this->fpdf->Cell($tamañoPapel - $tab - 33, 10, $this->utf8_decode($data['fullName']), 1, 0, 'L');
        ...
        $this->fpdf->Cell($tab + 20, 10, $this->utf8_decode($data['gender']), 1, 0, 'L');
        ...
        $this->fpdf->Cell($tab + 20, 10, $this->utf8_decode($data['ci']), 1, 0, 'L');
        $this->fpdf->SetX($tamañoPapel / 2);
        $this->fpdf->Cell($tab + 25, 10, $this->utf8_decode("Pasaporte:"), 1, 0, 'L');
        $this->fpdf->Cell($tab + 11, 10, $this->utf8_decode($data['passport']), 1, 0, 'L');
        ...
        $this->fpdf->SetX($tamañoPapel / 2);
        $this->fpdf->Cell($tab + 25, 10, $this->utf8_decode("E-mail:"), 1, 0, 'L');
        $this->fpdf->Cell($tab + 11, 10, $this->utf8_decode($data['email']), 1, 0, 'L');

        $height = 146;
        $this->fpdf->SetY($height);
        $this->fpdf->Ln(5);
        $this->fpdf->SetX(10);
        $this->fpdf->Cell($tab + 10, 10, $this->utf8_decode("Profesión:"), 1, 0, 'L');
        $this->fpdf->Cell($tab + 20, 10, $this->utf8_decode($data['profession']), 1, 0, 'L');
        $this->fpdf->SetX($tamañoPapel / 2);
        $this->fpdf->Cell($tab + 25, 10, $this->utf8_decode("Universidad de Origen:"), 1, 0, 'L');
        $this->fpdf->Cell($tab + 12, 10, $this->utf8_decode($data['university']), 1, 0, 'L');
        ...
        $this->fpdf->Cell($tab + 20, 10, $this->utf8_decode($data['graduationYear']), 1, 0, 'L');
        $this->fpdf->SetX($tamañoPapel / 2);
        $this->fpdf->Cell($tab + 25, 10, $this->utf8_decode("N° de Registro en la UAGRM:"), 1, 0, 'L');
        $this->fpdf->Cell($tab + 12, 10, $this->utf8_decode($data['registrationNumber']), 1, 0, 'L');
        ...
        $this->fpdf->Cell($tamañoPapel - 83, 10, $this->utf8_decode($data['workExperience']), 1, 0, 'L');
        ...
        $this->fpdf->SetX(10);
        $this->fpdf->SetFont('Arial', '', 8);
        $this->fpdf->MultiCell($tamañoPapel - 21, 10, $this->utf8_decode("Gracias por compartirnos su experiencia laboral: \n " . $data["workExperience"]), 1, 'J');
